...
class ChatScenario extends BaseScenario {
  ...
  Future<void> backToChatLisFromChatGroupScreen(
      {bool isOpenGroupFromSearchResult = false}) async {
  ...
  PatrolFinder getPinExpandIcon() {
    return $(PinnedEventsView).$(DediIconButton).first;
  }
  ...
  Future<ChatGroupDetailRobot> createANewGroupChat(
      String groupName, List<String> memberAccounts,
  ...
  Future<void> openPinnedPanel(PatrolIntegrationTester $) async {
    ...
    await $.waitUntilVisible($(PinnedMessagesScreen),
        timeout: const Duration(seconds: 30));
    await $.waitUntilVisible(
  ...
  Future<void> verifyTheDisplayInSelectedTextMode(
      String message, int selectedNumber) async {
  ...
  Future<void> verifyMessageIsPinned(String message,
      {bool expected = true}) async {
    final icon = getPinExpandIcon();
    ...
    if (expected) {
      ...
      expect(pinnedMsg.exists, isFalse,
