...
enum _EventPropertyMatchType {
  isValue,
  containsValue,
}

class _EventPropertyCondition {
  _EventPropertyCondition({
    required this.type,
    required this.field,
    required this.expectedValue,
  });

  final _EventPropertyMatchType type;
  final String field;
  final String expectedValue;

  factory _EventPropertyCondition.fromCondition(PushCondition condition) {
    if (condition.kind != 'event_property_is' &&
        condition.kind != 'event_property_contains') {
      throw Exception(
        'Logic error: invalid push rule passed to constructor ${condition.kind}',
      );
    }

    final field = condition.key?.trim();
    if (field == null || field.isEmpty) {
      throw Exception(
        'Push condition ${condition.kind} is missing key',
      );
    }

    final rawExpectedValue =
        condition.value ?? condition.pattern ?? condition.is$;
    if (rawExpectedValue == null) {
      throw Exception(
        'Push condition ${condition.kind} is missing value',
      );
    }

    return _EventPropertyCondition(
      type: condition.kind == 'event_property_is'
          ? _EventPropertyMatchType.isValue
          : _EventPropertyMatchType.containsValue,
      field: field,
      expectedValue: _normalizeValue(rawExpectedValue),
    );
  }

  bool match(Map<String, String> content) {
    final rawFieldValue = content[field];
    if (rawFieldValue == null) {
      return false;
    }

    final normalizedFieldValue = rawFieldValue.trim();
    switch (type) {
      case _EventPropertyMatchType.isValue:
        final decoded = _tryDecodeJson(normalizedFieldValue);
        if (decoded != null && decoded is! Map && decoded is! List) {
          return _normalizeValue(decoded) == expectedValue;
        }
        return normalizedFieldValue == expectedValue;
      case _EventPropertyMatchType.containsValue:
        ...
        if (decoded is List) {
          return decoded.any(
            (item) => _normalizeValue(item) == expectedValue,
          );
        }
...
class _OptimizedRules {
  ...
  List<_EventPropertyCondition> eventPropertyConditions = [];
      ...
      switch (condition.kind) {
        ...
        case 'event_property_contains':
          eventPropertyConditions
              .add(_EventPropertyCondition.fromCondition(condition));
          break;
        default:
  ...
  EvaluatedPushRuleAction? match(Map<String, String> event, String? displayName,
    ...
    if (eventPropertyConditions.any((condition) => !condition.match(event))) {
      return null;
    }
    if (matchDisplayname) {
